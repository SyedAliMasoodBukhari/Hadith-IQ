...
class HadithBO(AbsHadithBO):    
    def __init__(self, dalFascade: AbsDALFascade):
        ...
        self.name = "Omartificial-Intelligence-Space/arabic-labse-matryoshka" #SentenceTransformer("sentence-transformers/LaBSE")
    ...
    def importHadithFile(self, projectName, filePath) -> bool:
        try:
            bookcheck = self.dalFascade.associate_book_with_project(filePath, projectName)
            if bookcheck:
                print(f"Book from file {filePath} successfully associated with project '{projectName}'.")
                hadithcheck = self.dalFascade.associate_hadiths_with_project(filePath, projectName)
                if hadithcheck:
                    print(f"Hadiths from {filePath} successfully associated with project '{projectName}'.")
                    sanadcheck = self.dalFascade.associate_sanads_with_project_by_book(filePath, projectName)
                    if sanadcheck:
                        print(f"Sanads from {filePath} successfully associated with project '{projectName}'.")
                        return True  
                    else:
                        print(f"Failed to associate sanads from file {filePath} with project '{projectName}'.")
                else:
                    print(f"Failed to associate hadiths from file {filePath} with project '{projectName}'.")
            else:
                print(f"Failed to associate book from file {filePath} with project '{projectName}'.")
    ...
    # Process all data in parallel
        ...
        partial(HadithBO._generateEmbeddings, token=self.tokenizer,labse_model=self.labse_model),
        )#change here
